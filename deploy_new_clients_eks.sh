...
PGPASSWORD="$DB_ENV_POSTGRES_PASSWORD" /usr/bin/psql -X -A --quiet --host $DB_PORT_5432_TCP_ADDR --port=5432 --username="$DB_ENV_POSTGRES_USER" -t -c "CREATE USER \"$psql_user\" WITH PASSWORD '$psql_user_password'; "
error=$?; if [ $error -eq 0 ]; then echo "Postgres User created Succesfully"; else echo "ERROR: $error"; fi


PGPASSWORD="$DB_ENV_POSTGRES_PASSWORD" /usr/bin/psql -X -A --quiet --host $DB_PORT_5432_TCP_ADDR --port=5432 --username="$DB_ENV_POSTGRES_USER" -t -c "ALTER ROLE \"$psql_user\" WITH createdb; "
